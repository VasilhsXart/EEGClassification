...
#LSTM model
# model = Sequential([
#     Input(shape=(max_samples, channels)),
...
# model = Sequential([
#     Input(shape=(max_samples, channels)),
#     Conv1D(filters=32, kernel_size=8, padding='valid'),
#     Activation('elu'),
#     MaxPooling1D(pool_size=4),
#     BatchNormalization(),
#     SpatialDropout1D(0.5),
#     Conv1D(filters=64, kernel_size=8, padding='valid'),
#     Activation('elu'),
#     MaxPooling1D(pool_size=4),
#     BatchNormalization(),
#     SpatialDropout1D(0.5),
...
history = model.fit(data_generator(X_train, Y_train, batch_size=batch_size),steps_per_epoch=steps_per_epoch,epochs=10,verbose=1,validation_data=data_generator_evaluate(X_val, Y_val, batch_size=batch_size),validation_steps=validation_steps)
...
print("------Evaluation------")
#Evaluate the model
loss, accuracy = model.evaluate(data_generator_evaluate(X_test,Y_test,batch_size=batch_size),verbose=1,steps=steps)
print(f'Test Loss: {loss}, Test Accuracy: {accuracy}')

#Predict the outcomes of the model
Y_pred = model.predict(data_generator_evaluate(X_test,Y_test, batch_size=batch_size), steps=steps)
