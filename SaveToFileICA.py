    ...
    if os.path.isdir(folder_path):
            ...
            for file in os.listdir(folder_path):
                if file.endswith('.edf'):
                    ...
                    rawEDF.filter(l_freq=1.0, h_freq=40.0)
                    rawEDF.resample(160, npad='auto')

                    # Fit ICA
                    ica = ICA(n_components=10, random_state=97,method='infomax')
                    ica.fit(rawEDF)

                    # Apply ICA
                    rawEDF_clean = ica.apply(rawEDF)

                    #Read the annotations of the signal and split based on duration
                    rawEvents = mne.events_from_annotations(rawEDF_clean)
                    durations = rawEDF_clean.annotations.duration
                    annotations = rawEDF_clean.annotations.description
                    tmin = 0
                    for i in range(len(durations)):
                        try:
                            chunk = rawEDF_clean.copy().crop(tmin, tmin+durations[i])
                            tmin += durations[i]
                            ...
                            chunk = rawEDF_clean.copy().crop(tmin, tmin+durations[i] - 0.01)
                            chunk_data = chunk.get_data()
                            label = extract_labels(file_path,annotations[i])-1
                            if label not in Labels_Used:
                                Labels_Used.append(label)
